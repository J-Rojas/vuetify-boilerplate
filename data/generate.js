...
export const articles = (generate = 9) => {
  ...
  for (let i = 1; i <= generate; i++) {
    ...
    const indexArticleType = i > countArticleTypes ? i - countArticleTypes : i
    ...
    elems.push({
      ...
      mainImageId:
        indexArticleType !== articleTypesEnum.AWARD && (numberIsOdd(i) || indexArticleType === articleTypesEnum.USER)
          ? faker.image.imageUrl(800, 400, 'animals', true, true)
          : null,
      numberOfAttachments:
        indexArticleType !== articleTypesEnum.AWARD && indexArticleType !== articleTypesEnum.USER && !numberIsOdd(i)
      ...
      priorityId:
        indexArticleType !== articleTypesEnum.AWARD && numberIsOdd(i)
          ? faker.datatype.number({ min: 0, max: 4 })
          : null,
      published: true,
  ...
  return elems && elems.length
    ? elems.sort((a, b) => new Date(b.publishedDate).getTime() - new Date(a.publishedDate).getTime())
    : []
}
...
export const feedback = (generate = 3) => {
  ...
  return elems && elems.length
    ? elems.sort((a, b) => new Date(b.createdOn).getTime() - new Date(a.createdOn).getTime())
    : []
}
...
export const objectives = (generate = 3) => {
  ...
  return elems && elems.length
    ? elems.sort((a, b) => new Date(a.endDate).getTime() - new Date(b.endDate).getTime())
    : []
}
...
export const people = (generate = 150) => {
  ...
  return elems && elems.length ? elems.sort((a, b) => a.fullName.localeCompare(b.fullName)) : []
}
